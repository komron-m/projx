...
const getAccountsByID = `-- name: GetAccountsByID :many
SELECT id, owner, balance, currency, created_at
FROM accounts
...
func (q *Queries) GetAccountsByID(ctx context.Context, ids []int64) ([]*Account, error) {
	rows, err := q.db.Query(ctx, getAccountsByID, ids)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
